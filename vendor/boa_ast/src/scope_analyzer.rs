...
struct BindingCollectorVisitor<'interner> {
    ...
    scope: Scope,
    in_arrow: bool,
    /// Nearest non-arrow function whose `this` is inherited by nested arrows.
    this_scope: Option<Scope>,
    interner: &'interner Interner,
}
...
impl<'ast> VisitorMut<'ast> for BindingCollectorVisitor<'_> {
    ...
    fn visit_this_mut(
        ...
        if self.in_arrow {
            if let Some(scope) = &self.this_scope {
                scope.escape_this();
            }
        }
        ControlFlow::Continue(())
...
impl BindingCollectorVisitor<'_> {
    ...
    fn visit_function_like(
        ...
        // Arrow functions do not bind `this`. Retain the nearest ordinary function across any
        // number of nested arrows so a lexical `this` access preserves the environment that
        // actually owns the receiver.
        if !arrow {
            self.this_scope = Some(function_scope.clone());
        }
