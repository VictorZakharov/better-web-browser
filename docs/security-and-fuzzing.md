...
| CSS | 4 MiB/source; 20,000 rules; nesting 64; 256 declarations/rule | Truncate source and stop bounded parser loops while retaining valid rules already parsed. |
...
## Interpreting JavaScript execution failures

The V8 execution deadline aborts the current invocation; it does not automatically destroy the
retained document realm. Later timer tasks can still run. Changes made to JavaScript state and the
DOM before interruption remain, and an interrupted one-shot timer is not retried. Consequently, a
site can remain partially updated even while its other scripts, network requests, or audio continue.

The benchmark field `javascript_runtime_stopped` reports realm cancellation or an evaluator failure
that stopped the runtime, not whether every task succeeded. A value of `false` alongside an
