...
[September 16 DOMParser/hidden-removal reassessment](browser-performance-domparser-2026-09-16.md).
The matrix has since expanded from nine to fourteen fixtures. The current source of truth is
...
The results establish an explicit gate for the owned feature-equivalent paths. They do not establish whole-web compatibility or a universal performance advantage. Chromium implements a much larger platform; Breeze has one end-to-end non-DRM H.264/AAC media path and a bounded Canvas 2D pixel slice but still lacks broader Canvas drawing operations, media formats and DRM, accessibility, cross-site isolation, and broad standards coverage. The curated WPT gate and documented remaining compatibility boundaries track standards support without conflating a passing selection with global conformance.
