...
const APP_NAME: &str = "Breeze";
const MAIN_CLASS: &str = "BreezeFastBrowserWindow";
const TASK_CLASS: &str = "BreezeTaskManagerWindow";
...
pub fn show_fatal_error(error: &str) {
    ...
    let title = wide("Breeze failed to start");
    unsafe {
...
impl BrowserState {
    fn new(instance: Hinstance, metrics: Arc<BrowserMetrics>, options: LaunchOptions) -> Self {
        let home = parse_html(
            r#"
                <title>Breeze</title>
                <h1>A small, document-first browser</h1>
                <p>Enter an address or search above. Breeze downloads HTML, removes scripts and styles, and lays out readable content without a heavyweight web runtime.</p>
                <h2>MVP capabilities</h2>
                <ul>
                    <li>Native HTTPS navigation</li>
                    <li>Headings, paragraphs, lists, quotes, preformatted text, and clickable links</li>
                    <li>Back, forward, reload, scrolling, and search</li>
                    <li>A built-in task manager for CPU and memory telemetry</li>
                </ul>
                <p>Try <a href="https://example.org/">example.org</a> or search for something.</p>
            "#,
            "https://breeze.local/",
        );
        Self {
    ...
    unsafe fn resize_controls(&mut self) {
        ...
        MoveWindow(self.controls.address, 152, 9, (width - 340).max(50), 31, 1);
        MoveWindow(self.controls.go, width - 182, 9, 48, 31, 1);
    ...
    unsafe fn begin_navigation(&mut self, url: String, history_mode: HistoryMode) {
        ...
        std::thread::spawn(move || {
            ...
            let result = winhttp::get(&url).map(|response| {
                ...
                let html = winhttp::decode_text(&response.body);
    ...
    unsafe fn finish_navigation(&mut self, message: LoadMessage) {
        ...
        match message.result {
            Ok(page) => {
                ...
                set_window_text(
                    self.window,
                    &format!("{} — {APP_NAME}", self.document.title),
                );
                self.rebuild_layout();
                if let Some(benchmark) = self.benchmark.as_mut() {
    ...
    unsafe fn rebuild_layout(&mut self) {
        ...
        let (items, height) = layout_document(dc, fonts, &self.document, left, reading_width);
    ...
    unsafe fn open_task_manager(&mut self) {
        ...
        let title = wide("Breeze Task Manager");
        let window = CreateWindowExW(
            WS_EX_TOOLWINDOW,
            class.as_ptr(),
            title.as_ptr(),
