...
fn viewport_scroll_requests_round_trip_and_reject_non_finite_or_negative_offsets() {
    ...
    presentation.runtime.viewport_scroll_y = Some(120.25);
    let decoded = RendererPresentation::decode(&presentation.encode().unwrap()).unwrap();
    assert_eq!(decoded.runtime.viewport_scroll_y, Some(120.25));
    for invalid in [f32::NAN, f32::INFINITY, -1.0] {
        presentation.runtime.viewport_scroll_y = Some(invalid);
        assert!(presentation.encode().is_err());
    }
}

#[test]
fn malformed_wire_scroll_offsets_are_rejected_before_history_allocation() {
    ...
    for invalid in [f32::NAN, f32::INFINITY, -1.0] {
        ...
        writer.bool(false); // no navigation URL
        writer.bool(true); // scroll request present
        writer.f32(invalid);
        let bytes = writer.finish();
        let error = codec::decode_runtime(&mut WireReader::new(&bytes)).unwrap_err();
        assert!(matches!(
            error,
            ProtocolError::InvalidPayload("viewport scroll offset")
        ));
    }
}
