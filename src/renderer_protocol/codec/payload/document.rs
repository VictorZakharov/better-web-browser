...
pub(super) fn encode_browser_document(
    ...
    let kind = match message {
        BrowserMessage::BeginStreamingDocument(start) => {
            if start.body_length != 0 {
                return Err(ProtocolError::InvalidPayload("streaming document length"));
            }
        ...
        BrowserMessage::AbortDocument { document, message } => {
            if message.len() > 16 * 1024 {
                return Err(ProtocolError::InvalidPayload("navigation error length"));
            }
...
pub(super) fn decode_browser_document(
    ...
    let message = match kind {
        0x0107 => {
            let start = decode_document_start(&mut reader)?;
            if start.body_length != 0 {
                return Err(ProtocolError::InvalidPayload("streaming document length"));
            }
            BrowserMessage::BeginStreamingDocument(start)
        }
        0x0109 => BrowserMessage::AbortDocument {
            document: DocumentId::new(reader.u64()?)?,
            message: reader.string(16 * 1024)?,
        },
