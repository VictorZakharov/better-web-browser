//! Guarded document-scoped IndexedDB result delivery.
use super::*;
use crate::renderer_protocol::{DatabaseCommand, DatabaseEvent};
use std::panic::{AssertUnwindSafe, catch_unwind};

impl ChildConnection {
    pub(in crate::renderer_process::child) fn send_database_command(
        &mut self,
        command: DatabaseCommand,
    ) -> Result<(), String> {
        command.validate().map_err(|error| error.to_string())?;
        self.writer
            .send_renderer(&RendererMessage::DatabaseCommand(command))
            .map_err(|error| error.to_string())
    }

    pub(super) fn deliver_database_event(&mut self, event: DatabaseEvent) -> Result<(), String> {
