    ...
    class IDBCursor {
        ...
        get primaryKey() { return this.key; }
        _setRecord(record) {
            this._record = record;
        ...
        _step(after, inclusive, skip) {
            ...
            this.source.transaction._enqueue(this.source, {
        ...
        update(value) {
            ...
            if (!this._record)
                throw new DOMException('Cursor has no current record', 'InvalidStateError');
            if (this.source.keyPath === null) return this.source.put(value, this.primaryKey);
            const inline = idbPathValue(value, this.source.keyPath);
            if (inline === undefined || idbCompare(idbKey(inline), this._record.key) !== 0)
