...
| ◩ | Canvas, media, and downloads | The contained media worker provides user-visible non-DRM H.264/AAC MP4 playback, synchronized XAudio2 output, progressive and bounded Media Source input, play/pause/seek/volume/mute controls, and fullscreen video. `canPlayType()`, `MediaSource.isTypeSupported()`, and `MediaCapabilities.decodingInfo()` advertise only that tested path; other codecs and encrypted media fail closed. Canvas, downloads, broader codec/container coverage, DRM, captions, track selection, and picture-in-picture remain incomplete. |
