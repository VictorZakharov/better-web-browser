...
- Before handing any pull request to the user for review, rebuild the local release browser from the final PR head with `cargo build --release --locked --bin better-web-browser` and report the exact executable path. If the branch changes after that build, rebuild it again before handoff.
